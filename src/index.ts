...
class TagComponent extends React.Component {
  render() {
    return (React.createElement("h1",null, "Wassup!"));
  }
}
...
class TagWidget extends Widget {
  constructor(notebook_Tracker: INotebookTracker) {
    ...
    this.tagComponent = new TagComponent(null, null);
    console.log(this.tagComponent);
  }
  private notebookTracker: INotebookTracker = null;
  private tagComponent: TagComponent = null;
  
}*/

function clickAddTag(event: React.FormEvent<HTMLInputElement>) {
  write_tag(cell, event.target.value, true);
}

function getWidget(notebookTracker: INotebookTracker):Widget{
  var CellTools= React.createElement(
      "div", {},
      React.createElement("h3",{},"Cell Tools"),
      React.createElement(
        "input",
        {
          type: "text",
          name: "addTag",
          onSubmit: (event) => clickAddTag(event)
        }, "Add Tag"
    ));
  let app:Widget = new ReactElementWidget(CellTools);
  return app;
}
...
class TagsTool extends CellTools.Tool {
  constructor(notebook_Tracker: INotebookTracker, app: JupyterLab) {
    ...
    this.notebookTracker = notebook_Tracker;
    let layout = this.layout = new PanelLayout();
    this.widget = getWidget(this.notebookTracker);
    layout.addWidget(this.widget);
  }
